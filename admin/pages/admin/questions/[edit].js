...
import {
  ...
  HiPhotograph, HiCalculator, HiPencil
} from 'react-icons/hi'
...
export default function EditQuestion() {
  ...
  const { edit: questionId } = router.query
  ...
  async function checkUserAndFetchData() {
    try {
      ...
      await Promise.all([fetchExams(), fetchQuestionData()])
    } catch (error) {
  ...
  async function fetchQuestionData() {
    try {
      ...
      setFormData({
        ...
        month: question.month?.toString() || '',
        shift: question.shift || '',
        units: question.units || '',
        tolerance: question.tolerance || '0',
        customTolerance: question.custom_tolerance || ''
      })
      ...
      if (question.question_type === 'objective' && question.options) {
        try {
          ...
          const newOptions = parsedOptions.map((opt, index) => ({
            ...
            content: opt.blocks?.[0]?.content || ''
          }))
          // Fill remaining slots with empty options
          while (newOptions.length < 4) {
            const nextId = String.fromCharCode(65 + newOptions.length)
            newOptions.push({ id: nextId, content: '' })
          }
          ...
          <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-6">
            <div className="flex items-center space-x-4">
              ...
              <div>
                ...
                <p className="text-gray-400 mt-2">
                  Update question details, content, and solutions
                </p>
              </div>
            </div>
            <div className="flex items-center space-x-3">
              ...
              <button
                ...
                className="px-4 py-2 bg-red-900/30 border border-red-700 text-red-300 rounded-lg hover:bg-red-800/30 flex items-center space-x-2"
              >
              ...
              <form onSubmit={handleSubmit} className="space-y-8">
                      ...
                      {optionsArray.map((option) => (
                        <div key={option.id} className="space-y-2">
                          <div className="flex items-center justify-between">
                            ...
                            {option.content.trim() && optionsArray.length > 2 && (
                              <button
                                type="button"
                                onClick={() => removeOption(optionsArray.indexOf(option))}
                ...
                <div className="pt-6 border-t border-gray-700 flex space-x-4">
                  <button
                    ...
                    className="flex-1 py-3.5 px-4 bg-gray-700 hover:bg-gray-600 text-white font-semibold rounded-xl transition-all duration-300"
                  >
                    Cancel
                  </button>
                  <button
          ...
          <div>
            <div className="bg-gray-800/30 backdrop-blur-sm rounded-2xl border border-gray-700 p-6 sticky top-8">
              ...
              <div className="mb-6 p-4 bg-gray-900/50 rounded-lg">
                <h4 className="text-sm font-semibold text-gray-300 mb-3">Question ID</h4>
                ...
                <h4 className="text-sm font-semibold text-gray-300 mb-2">Question Type</h4>
                <div className={`px-3 py-1 rounded-full text-xs font-medium inline-block ${
                  formData.question_type === 'objective' 
                    ? 'bg-blue-900/30 text-blue-300 border border-blue-700'
                    : 'bg-purple-900/30 text-purple-300 border border-purple-700'
                }`}>
                  {formData.question_type === 'objective' ? 'Objective (MCQ)' : 'Numerical'}
                </div>
              </div>
              ...
              <div className="pt-6 border-t border-gray-700">
                ...
                <div className="space-y-2">
                  ...
                  >
                    <div className="flex items-center justify-between">
                      <span className="text-gray-300">Back to Questions</span>
                      <HiArrowLeft className="w-4 h-4 text-gray-300" />
                    </div>
                  </Link>
                  <Link
                  ...
                  >
                    <div className="flex items-center justify-between">
                      <span className="text-gray-300">Create New Question</span>
                      <HiPlus className="w-4 h-4 text-gray-300" />
                    </div>
                  </Link>
                </div>
              </div>
              ...
              <div className="mt-6 p-4 bg-yellow-900/20 border border-yellow-700/50 rounded-lg">
                <h4 className="text-sm font-semibold text-yellow-300 mb-2">Warning</h4>
                <p className="text-sm text-yellow-200/80">
                  Deleting this question is permanent and cannot be undone. All associated data will be removed.
                </p>
              </div>
            </div>
          </div>
